function [theta] = Angle_Control(ti, MRobots, ORobots, z, tau, q, eta, MF, OF, NMA, NOA, dt)
    % Missing to remove the opponents that have a timeout or are tagged
    Oponnent_Positions = Positions_in_Matrix(MRobots.(z)(ti,1), ORobots, NOA, ti); %Matrix NOAx2 where the positions of the NOA opponents are the rows 
    ...
    theta = sin(ti*dt*4);
        ...
        elseif (active_opponents > 0) && MRobots.(eta)(ti) == 0 && MRobots.(q)(ti) == 0  && MRobots.(z)(ti,1)*MF(1)> 0
            if MRobots.mu(ti) == 1
                % Defend the flag (Capture the closest attacker)
                xst = 0.5*(norm(ORobots.(cz)(ti,1:2)-MF)^2-norm(MRobots.(z)(ti,1:2)-MF)^2)*(ORobots.(cz)(ti,1)-MRobots.(z)(ti,1))/(norm(ORobots.(cz)(ti,1:2)-MRobots.(z)(ti,1:2))^2);
                yst = 0.5*(norm(ORobots.(cz)(ti,1:2)-MF)^2-norm(MRobots.(z)(ti,1:2)-MF)^2)*(ORobots.(cz)(ti,2)-MRobots.(z)(ti,2))/(norm(ORobots.(cz)(ti,1:2)-MRobots.(z)(ti,1:2))^2);
               theta = acos((xst-(MRobots.(z)(ti,1)-MF(1)))/norm([xst,yst]-(MRobots.(z)(ti,1:2)-MF)));
            else 
